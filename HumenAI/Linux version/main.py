...
def window1():
    ...
    def save():
        try:
            if os.path.exists(f"/home/{user}/Изображения"):
                shutil.copy2(f'runs/detect/predict/{file0}', f'/home/{user}/Изображения/{file0}')
                save_button['text']=f'Изображение сохранено в Изображения(/home/{user}/Изображения/{file0})'
            else:
                shutil.copy2(f'runs/detect/predict/{file0}', f'/home/{user}/Pictures/{file0}')
                save_button['text']=f'Изображение сохранено в Pictures(/home/{user}/Pictures/{file0})'
        except Exception as e:
            label['text']=f'Ошибка: {e}'

    update_button = tk.Button(window, text="Загрузить изображение", font=("Arial", 13), command=pr)
    update_button.pack()
    save_button = tk.Button(window, text="Сохранить изображение", font=("Arial", 13), command=save)
    save_button.pack()
    back_button = Button(window,text="Назад", font=("Arial", 13),  command=back1)
    back_button.pack()
...
def start():
    ...
    window0.geometry('350x260')
    window0.resizable(0,0)
    window0.title('Выбор нужды')

    button_win1 = tk.Button(window0, text="Обработка изображений", font=("Arial", 14), command=window1)
    button_win1.pack()
    button_win2 = tk.Button(window0, text="Обработка видио", font=("Arial", 14), command=window2)
    button_win2.pack()
